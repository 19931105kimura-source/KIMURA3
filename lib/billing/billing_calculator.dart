...
/// ===============================
 class BillingCalculator {
  ...
  static const String _annaiSection = '案内所';
  static const int _roundUnitYen = 100;
  /// デフォルト率
  static const double defaultTaxRate = 0.10;      // 10%
  static const double defaultServiceRate = 0.25;  // 25% // 25%
 ...
 for (final item in items) {
  ...
  static BillingResult _buildResult(
    ...
    // ⑤ 100円未満切り捨て（伝票印刷側と揃える）
    final finalTotal = (rawTotal ~/ _roundUnitYen) * _roundUnitYen;
  ...
  // 課税判定（OrderItem）
   static bool _isTaxableItem(OrderItem item) {
     // セットは「案内所」だけ非課税（brand / section / label を見る）
    if (_isAnnaiSet(item.category, item.brand, item.section, item.label)) {
      return false;
    }
 ...
 static bool _isTaxableLine(OrderLine line) {
   // セットは「案内所」だけ非課税（brand / section / label を見る）
    if (_isAnnaiSet(line.category, line.brand, line.section ?? '', line.label)) {
      return false;
    }
    // それ以外は課税
    return true;
 ...
 static bool _isExcludedFromBilling(String category, [String subCategory = '']) {
  ...
  static bool _isAnnaiSet(
   ...
   final c = category.trim();
    if (c.isNotEmpty && c != 'セット') return false;
     return _containsAnnai(brand) ||
        _containsAnnai(section) ||
        _containsAnnai(label);
  }

  static bool _containsAnnai(String value) {
    return value.trim().contains(_annaiSection);
  }
}
