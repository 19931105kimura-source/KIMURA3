...
int _toInt(dynamic v) {
 ...
 TableTimerInfo? timerOf(String table) => tableTimers[table];
  ...
  DateTime? _resolveStartDateTime(String? hhmm, {DateTime? now}) {
    ...
    return DateTime(current.year, current.month, current.day, h, m);
  }

  int _calcRemainingSecondsFromClock({
    required String? startTime,
    required int totalSeconds,
    DateTime? now,
  }) {
    final start = _resolveStartDateTime(startTime, now: now);
    if (start == null) return totalSeconds < 0 ? 0 : totalSeconds;

    final current = now ?? DateTime.now();
    final safeTotal = totalSeconds < 0 ? 0 : totalSeconds;
    final endToday = start.add(Duration(seconds: safeTotal));
    final endPrevDay = start
 ...
 void clearTableTimer(String table) {
  ...
  Future<void> load() async {
   ...
   if (tableTimers.isNotEmpty) {
    ...
    // サーバー正本のテーブル一覧を取得（失敗時はローカル既定値を維持）
    await refreshTablesFromServer(notify: false);

    notifyListeners();
  }

  Future<void> refreshTablesFromServer({bool notify = true}) async {
    try {
      final uri = ServerConfig.api('/api/tables');
      final res = await http.get(uri);
      if (res.statusCode < 200 || res.statusCode >= 300) return;

      final body = jsonDecode(res.body);
      final raw = body is Map<String, dynamic> ? body['tables'] : null;
  ...
  Future<void> _addTableOnServer(String name) async {
    final uri = ServerConfig.api('/api/tables');
    final res = await http.post(
      uri,
    ...
    if (res.statusCode < 200 || res.statusCode >= 300) {
      throw Exception('add table failed: ${res.statusCode}');
    }
  }

  Future<void> _renameTableOnServer(String oldName, String newName) async {
    final oldEncoded = Uri.encodeComponent(oldName);
    final uri = ServerConfig.api('/api/tables/$oldEncoded');
    ...
    if (res.statusCode < 200 || res.statusCode >= 300) {
      throw Exception('rename table failed: ${res.statusCode}');
    }
  }

  Future<void> _removeTableOnServer(String name) async {
    final encoded = Uri.encodeComponent(name);
    final uri = ServerConfig.api('/api/tables/$encoded');
    final res = await http.delete(uri);
    if (res.statusCode < 200 || res.statusCode >= 300) {
      throw Exception('remove table failed: ${res.statusCode}');
    }
  }
...
if (order == null) {
 ...
 _lastSubmitError = null;
  ...
  Future<void> removeTable(String name) async {
    ...
    await _removeTableOnServer(name);
    _tables.remove(name);
  ...
  Future<void> renameTable(String oldName, String newName) async {
    ...
    await _renameTableOnServer(oldName, nn);
