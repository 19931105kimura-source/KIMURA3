...
const CMD = {
 ...
 // 機種差分対策で n は ASCII "1"/"0" を使う（LSB判定のため 0x01/0x00 でも理論上は同等）
  upsideDownOn:  Buffer.from([0x1b, 0x7b, 0x31]),  // ESC { '1'（上下逆ON）
  upsideDownOff: Buffer.from([0x1b, 0x7b, 0x30]),  // ESC { '0'（上下逆OFF）
...
function buildRawPrintData(segments, target = "receipt") {
  ...
  for (const seg of segments) {
    ...
    else if (seg.type === "upside_down") {
      chunks.push(seg.enabled ? CMD.upsideDownOn : CMD.upsideDownOff);
    }
