...
persistNormalizedOrderToDb({ orderId, tableId, orderedBy, orderedAt, items }) {
  ...
  // INSERT OR IGNORE → orderIdが既にあっても無視してスキップ（エラーにしない）
  const upsertOrder = db.prepare(`
    INSERT OR IGNORE INTO orders (id, table_id, created_at, status)
    VALUES (?, ?, ?, ?)
  `);

  // INSERT OR REPLACE → 同じitem IDなら上書きupsert
  ...
  const orderItems = (Array.isArray(items) ? items : []).map((it, idx) => ({
    orderItemId: String(it.orderItemId ?? it.id ?? `${normalizedOrderId}_${idx}`),
   ...
   printGroup: it.printGroup ?? null,
    shouldPrint: it.shouldPrint !== false,
    orderedBy: it.orderedBy ?? orderedBy ?? "guest",
    rtLineId: it.rtLineId ?? it.lineId ?? null,
    lineId: it.lineId ?? it.rtLineId ?? null,
  }));

  const tx = db.transaction(() => {
    upsertOrder.run(normalizedOrderId, normalizedTableId, timestamp, "open");
    ...
    for (const it of orderItems) {
     ...
     const meta = JSON.stringify({
        ...
        category: it.category,
        section: it.section,
          printGroup: it.printGroup,
        shouldPrint: it.shouldPrint,
        orderedBy: it.orderedBy,
        rtLineId: it.rtLineId,
        lineId: it.lineId ?? it.rtLineId,
      });
  ...
  replaceTableItemsInDb({ tableId, orderId, orderedBy, orderedAt, items }) {
    ...
    const tx = db.transaction(() => {
      ...
      sourceItems.forEach((it, idx) => {
        ...
        const orderItemId = `${normalizedOrderId}_${idx}_${Date.now()}`;
        const meta = {
          label: it.label ?? "",
          brand: it.brand ?? "",
          category: it.category ?? "",
  ...
  getTableSnapshot(tableId) {
   ...
   return rows.map((r) => {
      const meta = safeParseJsonObject(r.metaJson);
      return {
        ...meta,
        orderItemId: r.orderItemId,
        orderId: r.orderId,
        productId: r.productId,
        lineId: String(meta.lineId ?? meta.rtLineId ?? r.orderItemId),
        name: r.name,
...
if (removed === 0) {
    console.log("removeTableItem: line not found", {
      tableId,
      lineId,
    });
    throw new Error("Line not found");
 ...
 removeTableItemsByNamePrice(tableId, name, price) {
  ...
  buildRealtimeSnapshot() {
    ...
    const orderItems = {};
      ...
      // ★ tables（使用中/closed 等）を正本として全席を流す
    //    明細は常に DB を正本として扱う（fallback は使わない）
