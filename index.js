...
function clearTableRuntimeOrders(tableId) {
 ...
 if (store.tableOrders && typeof store.tableOrders.delete === "function") {
   store.tableOrders.delete(String(tableId));
 }
}

function findOrderItemByRtLine(tableId, rtLineId) {
  if (!rtLineId) return null;
  for (const [itemId, item] of store.orderItems.entries()) {
    if (item.tableId === tableId && item.rtLineId === String(rtLineId)) {
      return { itemId, item };
    }
  }
  return null;
...
function findOrderItemBySignature(tableId, candidate) {
  const tableKey = String(tableId);
  const candProductId = candidate?.productId ?? null;
  const candPrice = Number(candidate?.price ?? 0);
  const candName = String(candidate?.name ?? "").trim();
  const candLabel = String(candidate?.label ?? "").trim();
  const candBrand = String(candidate?.brand ?? "").trim();
  const candCategory = String(candidate?.category ?? "").trim();
  const candSection = candidate?.section == null ? null : String(candidate.section);
  const candSubCategory = String(candidate?.subCategory ?? "").trim();

  for (const [itemId, item] of store.orderItems.entries()) {
    if (String(item.tableId ?? "") !== tableKey) continue;

    const itemProductId = item.productId ?? null;
    if (candProductId || itemProductId) {
      if (String(itemProductId ?? "") !== String(candProductId ?? "")) continue;
      return { itemId, item };
    }
    ...
    const itemSection = item.section == null ? null : String(item.section);
    const itemSubCategory = String(item.subCategory ?? "").trim();

    const sameSnapshotLine =
      itemPrice === candPrice &&
      itemName === candName &&
      itemLabel === candLabel &&
      itemBrand === candBrand &&
      itemCategory === candCategory &&
      itemSection === candSection &&
      itemSubCategory === candSubCategory;
...
/////////オーダー伝票表示位置////////////
function formatOrderSlipTime(value) {
...
function buildOrderSlipSegments({ tableId, target, items, orderedAt }) {
  const ruleLine = makeRuleLine("ORDER_SLIP_RULE_WIDTH");////罫線の長さ
  const normalNameWidth = readIntEnv("ORDER_SLIP_NAME_WIDTH", 14);////品名幅
  const normalQtyWidth = readIntEnv("ORDER_SLIP_QTY_WIDTH", 4);/////数量幅
  const titleIndent = readIntEnv("ORDER_SLIP_TITLE_INDENT", 7);/////タイトルのインデント（スペース数）
  const bodyNameWidth = Math.max(4, Math.floor(normalNameWidth / 2));
  const bodyQtyWidth = Math.max(2, Math.floor(normalQtyWidth / 2));

  const headerLines = [];
  const bodyLines = [];
  const targetLabel = target === "register" ? "レジ" : "厨房";
  const timeLabel = formatOrderSlipTime(orderedAt);

  headerLines.push(ruleLine);
  headerLines.push(`${" ".repeat(titleIndent)}注 文 票（${targetLabel}）`);
  headerLines.push(ruleLine);
 
  if (timeLabel) {
    headerLines.push(`時刻：${timeLabel}`);
  }
  headerLines.push("");
    bodyLines.push(`席：${tableId}`);
  bodyLines.push("");
   // ↓ この部分が丸ごと抜けていた
  for (const item of items) {
    const name = printableItemName(item);
    const qty = Number(item.quantity ?? item.qty ?? 1);
    const namePart = padDisplayEnd(name, bodyNameWidth);
    const qtyPart = padDisplayStart(qty, bodyQtyWidth);
    bodyLines.push(`${namePart}${qtyPart}`);
}
 bodyLines.push("");
  return [
    { type: "text", text: `${headerLines.join("\n")}\n` },
    { type: "text_large", text: bodyLines.join("\n") },
  ];
}
async function printOrderSlip({ tableId, target, items }) {
  ...
  const segments = buildOrderSlipSegments({
    tableId,
 ...
 await enqueuePrint(
    [
      { type: "upside_down", enabled: true },
      ...segments,
      { type: "upside_down", enabled: false },
    ],
    normalizedTarget,
  );
...
// 共通：会計計算
// -------------------------
function floorToHundredYen(value) {
...
function isAnnaiSet(item) {
  const category = String(item?.category ?? "").trim();
   if (category && category !== "セット") return false;
...
function getTableItems(tableId) {
  ...
  // まずSQLite版を試す（実装済みならこちらを使う）
  if (typeof store.getTableItemsFromDb === "function") {
    try {
      const dbItems = store.getTableItemsFromDb(tableId);
      
       return dbItems;
    } catch (e) {
      console.error("getTableItemsFromDb failed, fallback to memory:", e?.message || e);
    }
  }
  ...
  for (const orderId of orderIds) {
    if (currentOrderId && orderId !== currentOrderId) continue;
...
function calcReceiptSummary(tableId) {
 ...
 for (const item of billableItems) {
     const annaiSet = isAnnaiSet(item);
  ...
  const total = floorToHundredYen(grossTotal);
...
function aggregateReceiptItems(items = []) {
  ...
  for (const item of items) {
    ...
    const price = Number(item?.price ?? 0);
    const taxClassKey = isAnnaiSet(item) ? "annai_set" : "taxable";
    const key = `${String(name).trim().toLowerCase()}|${price}|${taxClassKey}`;
...
function replaceTableItems(tableId, rawLines) {
  ...
  rawLines.forEach((it, idx) => {
    const stableLineId = String(it.lineId ?? "").trim();
    const itemId = stableLineId || `item_sync_${Date.now()}_${idx}`;
...
if (typeof store.replaceTableItemsInDb === "function") {
  ...
  normalizedItems.forEach((normalized) => {
    store.orderItems.set(normalized.id, normalized);
    itemIds.push(normalized.id);
  });
  store.orderItemsByOrder.set(orderId, itemIds);
  if (store.tableOrders && typeof store.tableOrders.set === "function") {
    store.tableOrders.set(String(tableId), {
      ...
      items: normalizedItems.map((item) => ({
        ...
        printGroup: normalizePrintTarget(item.printGroup ?? "kitchen"),
        shouldPrint: item.shouldPrint !== false,
        contributors: null,
        addedBy: item.orderedBy ?? "owner",
      })).filter((item) => item.lineId && item.qty > 0),
    });
  }
...
function persistRtTableToDb(tableId, { orderedBy = "owner" } = {}) {
  if (typeof store.replaceTableItemsInDb !== "function") return;
  if (typeof store.getTableOrderSnapshot !== "function") return;

  const snap = store.getTableOrderSnapshot(tableId);
  const sourceItems = Array.isArray(snap?.items) ? snap.items : [];
  const normalizedItems = sourceItems
    .map((item, idx) => {
      const qty = Number(item.qty ?? item.quantity ?? 0);
      if (!Number.isFinite(qty) || qty <= 0) return null;

      const lineId = String(item.lineId ?? `rt_${Date.now()}_${idx}`);
      const price = Number(item.price ?? 0);
      const printGroup = normalizePrintTarget(
        item.printGroup ?? item.printTarget ?? "kitchen"
      );

      return {
        id: lineId,
        productId: item.productId ?? null,
        name: item.name ?? item.label ?? item.brand ?? "",
        label: item.label ?? item.name ?? "",
        brand: item.brand ?? "",
        category: item.category ?? "",
        section: item.section ?? null,
        ...
        printGroup,
        shouldPrint: item.shouldPrint !== false,
        orderedBy: item.addedBy ?? orderedBy,
      };
    })
    .filter(Boolean);
  ...
  function hydrateRtSnapshotFromDb(tableId) {
  if (typeof store.getTableItemsFromDb !== "function") return false;
  if (!store.tableOrders || typeof store.tableOrders.set !== "function") return false;

  const dbItems = store.getTableItemsFromDb(tableId);
  if (!Array.isArray(dbItems) || dbItems.length === 0) return false;

  const hydrated = dbItems.map((item) => ({
    lineId: String(item.lineId ?? item.orderItemId ?? ""),
    productId: item.productId ?? null,
    name: item.name ?? item.label ?? "",
    label: item.label ?? item.name ?? "",
    brand: item.brand ?? "",
...
store.openTable(tableId);
  ...
  // ② テーブル用の canonical orderId（初回のみ作成）
  const orderId = ensureTableOrderId(tableId);
  ...
  // ⑤ item を 1件ずつ canonical line に upsert
  //    ※ printOrderSlip は今回追加分（delta）を使う
...
try {
    rawItems.forEach((it, idx) => {
      ...
      const tableLine = it.productId
       ...
       const rtLineId = tableLine?.lineId ?? null;
      const found =
        findOrderItemByRtLine(tableId, rtLineId) ||
        findOrderItemBySignature(tableId, {
          productId: it.productId ?? null,
          name,
          label: it.label ?? "",
          brand: it.brand ?? "",
      ...
      if (found) {
        canonicalItemId = found.itemId;
        found.item.quantity = Number(tableLine?.qty ?? quantity);
        found.item.price = price;
        found.item.name = name;
        found.item.label = it.label ?? found.item.label ?? "";
        found.item.brand = it.brand ?? found.item.brand ?? "";
        found.item.category = it.category ?? found.item.category ?? "";
        found.item.section = it.section ?? found.item.section ?? null;
        found.item.subCategory = it.subCategory ?? found.item.subCategory ?? "";
        found.item.printTarget = printTarget;
        found.item.printGroup = printGroup;
        found.item.orderedBy = orderedBy;
        found.item.shouldPrint = it.shouldPrint !== false;
        found.item.rtLineId = rtLineId;
      } else {
        const normalized = {
          id: itemId,
          orderId,
          ...
          subCategory: it.subCategory ?? "",
          price,
          quantity: Number(tableLine?.qty ?? quantity),
          printTarget,
          printGroup,
          printed: { kitchen: false, register: false, receipt: false },
          orderedBy,
      ...
      if (!itemIds.includes(canonicalItemId)) {
        itemIds.push(canonicalItemId);
      }
...
app.post('/api/orders/sync-table', (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ success: false, error: 'sync-table failed' });
  }
});
...
app.post("/api/rt/tables/:tableId/items", async (req, res) => {
  try {
    ...
    const found = findOrderItemByRtLine(tableId, line.lineId);
    let canonicalItemId = itemId;
    if (found) {
      canonicalItemId = found.itemId;
      found.item.quantity = Number(line.qty ?? found.item.quantity ?? 1);
      found.item.price = Number(line.price ?? found.item.price ?? 0);
      found.item.name = line.name ?? found.item.name;
      found.item.label = product?.variantLabel ?? found.item.label ?? "";
      found.item.brand = product?.name ?? line.name ?? found.item.brand;
      found.item.category = product?.category ?? found.item.category ?? "";
      found.item.printTarget = printTarget;
      found.item.printGroup = normalizedPrintTarget;
      found.item.orderedBy = line.addedBy ?? found.item.orderedBy ?? "guest";
      found.item.shouldPrint = shouldPrint !== false;
    } else {
    ...
    // RT更新をDBにも即時反映（表示系のDB優先読みに整合させる）
...
app.patch("/api/rt/tables/:tableId/items/:lineId", (req, res) => {
  try {
    ...
    let line;
    try {
      line = store.updateTableItemQty(tableId, lineId, qty);
    } catch (e) {
      if (e?.message === "Line not found" && hydrateRtSnapshotFromDb(tableId)) {
        line = store.updateTableItemQty(tableId, lineId, qty);
      } else {
        throw e;
      }
   ...
   for (const item of store.orderItems.values()) {
      const matchedLine =
        item.rtLineId === String(lineId) || item.id === String(lineId);
      if (item.tableId === tableId && matchedLine) {
...
app.post("/api/rt/tables/move", (req, res) => {
  try {
    ...
    // ① DBから元の注文取得
    const fromItems = store.getTableItemsFromDb(from);

    // ② 移動先チェック（DB基準）
    const toItems = store.getTableItemsFromDb(to);
    if (toItems.length > 0) {
      return res.status(400).json({ ok: false, error: "移動先に注文あり" });
    }

    // ③ 移動先にコピー（DB）
    store.replaceTableItemsInDb({
      tableId: to,
      orderId: `rt_${to}_${Date.now()}`,
      orderedBy: "owner",
      orderedAt: new Date().toISOString(),
      items: fromItems,
    });
    ...
    // ⑤ テーブル状態更新
    const fromTable = store.getTable(from);
    const toTable = store.getTable(to);

    if (toTable) {
      toTable.status = "ordering";
   ...
   if (fromTable) {
      fromTable.status = "closed";
      fromTable.closedAt = new Date().toISOString();
    }

    // ⑥ runtime（tableOrders / orderItems / ordersByTable）もDB基準へ同期
    // ここをしないと、移動直後の追加注文で stale な tableOrders を正本として
    // DB を再置換し、移動済み明細が消えることがある。
    clearTableRuntimeOrders(to);
    clearTableRuntimeOrders(from);
    hydrateRtSnapshotFromDb(to);
...
console.log("DATA PATH:", path.resolve("data/menu.json"));